//
//  MyOwnedRewardViewController.swift
//  LinkIDApp
//
//  Created by Phan Tuan Anh on 28/03/2024.
//

import UIKit

class MyOwnedRewardViewController: BaseViewController {

    class func create(with navigator: Navigator, viewModel: MyOwnedRewardViewModel) -> Self {
        let vc = UIStoryboard.myReward.instantiateViewController(ofType: MyOwnedRewardViewController.self)
    ...
    var viewModel: MyOwnedRewardViewModel!
    ...
    @IBAction func onFilterAction(_ sender: Any) {
        self.navigator.show(segue: .myrewardFilter(myRewardType: MyRewardType.myOwnedReward, filterModel: viewModel.output.filterModel.value, applyFilterAction: { filterModel in
...
extension MyOwnedRewardViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        _tableView.backgroundView = (viewModel.output.isLoadingListReward.value == false && viewModel.output.isRefreshing.value == false && itemCount == 0) ? _tableEmptyView: nil
        return itemCount
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.setDataForCell(data: gift, isLastItem: isLastItem)
        return cell
    }
